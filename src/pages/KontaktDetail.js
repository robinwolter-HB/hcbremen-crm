...
export default function KontaktDetail() {
  ...
  async function load() {
    const [{ data: k },{ data: ap },{ data: h },{ data: s },{ data: t },{ data: ev },{ data: n }] = await Promise.all([
  ...
  async function deleteAP(apid) {
    ...
    await supabase.from('ansprechpartner').delete().eq('id', apid)
    load()
  }
  ...
  return (
    <main className="main">
      <button className="back-btn" onClick={() => navigate('/kontakte')}>&#8592; Zurueck zur Liste</button>
      <div className="card">
        ...
        <div className="tabs">
          {[['info','Kontaktdaten'],['ansprechpartner','Ansprechpartner ('+ansprechpartner.length+')'],['historie','Historie ('+historie.length+')'],['events','Events ('+events.length+')'],['sponsoring','Sponsoring']].map(([key,label]) => (
        ...
        {tab==='sponsoring' && (
          <div>
            {sponsoring
              ...
              : <div className="empty-state"><p>Kein Sponsoring-Vertrag hinterlegt.</p></div>
            }
          </div>
        )}
      </div>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <div style={{border:'1.5px solid var(--gray-200)',borderRadius:'var(--radius)',padding:10,display:'flex',flexWrap:'wrap',gap:8,minHeight:44}}>
                  {nutzer.length===0 && <span style={{color:'var(--gray-400)',fontSize:13}}>Keine Nutzer geladen</span>}
                  {nutzer.map(n => {
                    const name = n.name || n.email
                    const selected = (hForm.zustaendig_personen||[]).includes(name)
                    return (
                      <button key={n.email} type="button" onClick={()=>toggleZustaendig(name)}
