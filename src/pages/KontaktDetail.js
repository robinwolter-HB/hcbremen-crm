...
export default function KontaktDetail() {
  ...
  async function saveKontakt() {
    setSaving(true)
    const payload = { firma:kForm.firma, email:kForm.email||null, telefon:kForm.telefon||null, website:kForm.website||null, branche:kForm.branche||null, status:kForm.status, kategorie:kForm.kategorie, zustaendig:kForm.zustaendig||null, notiz:kForm.notiz||null, adresse_strasse:kForm.adresse_strasse||null, adresse_plz:kForm.adresse_plz||null, adresse_stadt:kForm.adresse_stadt||null, geaendert_am:new Date().toISOString() }
  ...
  async function load() {
    ...
    setLinks(li||[]); setAnsprechpartner(ap||[]); setHistorie(h||[])
  ...
  async function loadKommentare(historieId) {
    const { data } = await supabase.from('historie_kommentare').select('*,profile(name,avatar_url)').eq('historie_id', historieId).order('erstellt_am', { ascending: true })
  ...
  async function addKommentar(historieId, kontaktFirma) {
    ...
    const { data: newKommentar } = await supabase.from('historie_kommentare').insert({ historie_id:historieId, autor_id:profile?.id, text, mentions }).select().single()
    if (mentions.length > 0 && newKommentar) {
      const { data: { session } } = await supabase.auth.getSession()
      fetch(`${process.env.REACT_APP_SUPABASE_URL}/functions/v1/send-mention`, { method:'POST', headers:{ 'Content-Type':'application/json', 'Authorization':`Bearer ${session?.access_token}` }, body:JSON.stringify({ kommentar_id:newKommentar.id, mentions, autor_name:profile?.name||profile?.email, kontakt_firma:kontaktFirma||kontakt?.firma, kommentar_text:text, kontakt_id:id }) }).catch(e => console.warn('Mention notification failed:', e))
    }
  ...
  function toggleHistorie(historieId) {
    setExpandedHistorie(prev => {
      ...
      if (newState[historieId] && !kommentare[historieId]) loadKommentare(historieId)
      return newState
    })
  }
  ...
  async function saveHistorie() {
    setSaving(true)
    const payload = { kontakt_id:id, erstellt_von:profile?.id, ansprechpartner:hForm.ansprechpartner||'', art:hForm.art||'Anruf', betreff:hForm.betreff||'', notiz:hForm.notiz||'', naechste_aktion:hForm.naechste_aktion||'', faellig_am:hForm.faellig_am||null, zustaendig:hForm.zustaendig||'', zustaendig_personen:hForm.zustaendig_personen||[], erledigt:hForm.erledigt||false, meeting_datum:hForm.meeting_datum||null, meeting_uhrzeit:hForm.meeting_uhrzeit||null, meeting_ort:hForm.meeting_ort||null, meeting_teilnehmer:hForm.meeting_teilnehmer||[] }
    let result
    ...
    if (result.error) { alert('Fehler beim Speichern: ' + result.error.message); setSaving(false); return }
  ...
  const LINK_KATEGORIEN = [{ key:'CI',label:'CI & Branding',icon:'🎨' },{ key:'Vertrag',label:'Vertrag',icon:'📄' },{ key:'Praesentation',label:'Präsentation',icon:'📊' },{ key:'Foto',label:'Fotos & Medien',icon:'📷' },{ key:'Website',label:'Website',icon:'🌐' },{ key:'Sonstiges',label:'Sonstiges',icon:'🔗' }]
      ...
      <div className="card">
                        ...
                        <div style={{fontSize:13}}>
                          ...
                          <span>
                            ...
                            {(()=>{
                              ...
                              return diff <= 30 ? <span style={{marginLeft:8,fontSize:11,background:'#fce4d6',color:'#d94f4f',padding:'1px 7px',borderRadius:10,fontWeight:700}}>🎉 in {diff} Tagen</span> : null
                            })()}
                          </span>
                        </div>
                      )}
            ...
            {historie.length===0?<div className="empty-state"><p>Noch keine Eintraege.</p></div>
              :<div style={{display:'grid',gap:12}}>
                {historie.map(h=>{
                  ...
                  return (
                    <div key={h.id} style={{border:'1.5px solid var(--gray-200)',borderLeft:`4px solid ${h.art==='Meeting'?'#8b5cf6':h.art==='E-Mail'?'#2d6fa3':h.art==='Anruf'?'#3a8a5a':h.erledigt?'var(--gray-200)':isUeberfaellig?'var(--red)':'var(--navy)'}`,borderRadius:'var(--radius)',overflow:'hidden',opacity:h.erledigt?0.65:1}}>
                      <div style={{display:'flex',alignItems:'flex-start',gap:12,padding:'12px 16px',background:isExpanded?'rgba(15,34,64,0.03)':'var(--white)',cursor:'pointer'}} onClick={()=>toggleHistorie(h.id)}>
                        <button onClick={e=>{e.stopPropagation();toggleErledigt(h)}} style={{background:'none',border:'none',cursor:'pointer',fontSize:22,flexShrink:0,marginTop:0}}>{h.erledigt?'✅':'⬜'}</button>
                        ...
                        <div style={{display:'flex',gap:6,flexShrink:0,alignItems:'center'}}>
                          <span style={{fontSize:12,color:'var(--gray-400)',display:'flex',alignItems:'center',gap:4}}>💬{hKommentare.length>0&&<span style={{background:'var(--navy)',color:'white',borderRadius:'50%',width:16,height:16,display:'flex',alignItems:'center',justifyContent:'center',fontSize:10}}>{hKommentare.length}</span>}{isExpanded?' ▲':' ▼'}</span>
                      ...
                      {isExpanded&&(
                        <div style={{borderTop:'1px solid var(--gray-100)',background:'var(--gray-100)'}}>
                              ...
                              {hKommentare.map(k=>(
                                <div key={k.id} style={{display:'flex',gap:10,alignItems:'flex-start'}}>
                                  <div style={{width:32,height:32,borderRadius:'50%',flexShrink:0,overflow:'hidden',border:'2px solid var(--gray-200)'}}>
                                    {k.profile?.avatar_url?<img src={k.profile.avatar_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<div style={{width:'100%',height:'100%',background:'var(--navy)',display:'flex',alignItems:'center',justifyContent:'center',color:'white',fontWeight:700,fontSize:13}}>{(k.profile?.name||'?')[0].toUpperCase()}</div>}
                                  </div>
                                  <div style={{flex:1,background:'var(--white)',borderRadius:'var(--radius)',padding:'8px 12px',border:'1px solid var(--gray-200)'}}>
                                    ...
                                    <p style={{fontSize:13,margin:0,lineHeight:1.5}}>{k.text.split(/(@\w[\w\s]*?)(?=[^\w\s]|$)/g).map((part,i)=>part.startsWith('@')?<span key={i} style={{background:'#ddeaff',color:'#1a4a8a',borderRadius:4,padding:'1px 4px',fontWeight:600}}>{part}</span>:part)}</p>
                                  </div>
                                </div>
                              ))}
                            </div>
                          ...
                          <div style={{padding:'10px 16px',display:'flex',gap:10,alignItems:'flex-start'}}>
                            <div style={{width:32,height:32,borderRadius:'50%',flexShrink:0,overflow:'hidden',border:'2px solid var(--gray-200)'}}>
                              {profile?.avatar_url?<img src={profile.avatar_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<div style={{width:'100%',height:'100%',background:'var(--navy)',display:'flex',alignItems:'center',justifyContent:'center',color:'white',fontWeight:700,fontSize:13}}>{(profile?.name||'?')[0].toUpperCase()}</div>}
                            </div>
                            <div style={{flex:1,display:'flex',gap:8}}>
                              <div style={{flex:1,position:'relative'}}>
                                <input value={kommentarText[h.id]||''} onChange={e=>{const val=e.target.value;setKommentarText(prev=>({...prev,[h.id]:val}));const atMatch=val.match(/@(\w*)$/);if(atMatch){const query=atMatch[1].toLowerCase();const sugg=personen.filter(p=>p.name.toLowerCase().includes(query)).slice(0,4);setMentionSuggestions(prev=>({...prev,[h.id]:sugg}));setShowMentions(prev=>({...prev,[h.id]:true}))}else{setShowMentions(prev=>({...prev,[h.id]:false}))}}} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey&&!showMentions[h.id]){e.preventDefault();addKommentar(h.id,kontakt?.firma)}if(e.key==='Escape')setShowMentions(prev=>({...prev,[h.id]:false}))}} placeholder="Kommentar... @Name zum Erwähnen, Enter zum Senden" style={{width:'100%',padding:'8px 12px',border:'1.5px solid var(--gray-200)',borderRadius:'var(--radius)',fontSize:13,background:'var(--white)',boxSizing:'border-box'}}/>
                                {showMentions[h.id]&&(mentionSuggestions[h.id]||[]).length>0&&(
                                  <div style={{position:'absolute',bottom:'100%',left:0,right:0,background:'var(--white)',border:'1.5px solid var(--gray-200)',borderRadius:'var(--radius)',boxShadow:'var(--shadow)',zIndex:10,marginBottom:4}}>
                                    {(mentionSuggestions[h.id]||[]).map(p=>(<div key={p.id} onClick={()=>{const val=kommentarText[h.id]||'';const newVal=val.replace(/@\w*$/,'@'+p.name+' ');setKommentarText(prev=>({...prev,[h.id]:newVal}));setShowMentions(prev=>({...prev,[h.id]:false}))}} style={{padding:'8px 12px',cursor:'pointer',display:'flex',alignItems:'center',gap:8,fontSize:13}} onMouseEnter={e=>e.currentTarget.style.background='var(--gray-100)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><div style={{width:24,height:24,borderRadius:'50%',background:'var(--navy)',display:'flex',alignItems:'center',justifyContent:'center',color:'white',fontSize:11,fontWeight:700,flexShrink:0}}>{p.name[0]}</div>{p.name}</div>))}
                                  </div>
                                )}
                              </div>
                              <button className="btn btn-sm btn-primary" onClick={()=>addKommentar(h.id,kontakt?.firma)} disabled={kommentarSaving||!kommentarText[h.id]?.trim()}>Senden</button>
                            </div>
                          </div>
                        </div>
                      )}
        ...
        {tab==='sponsoring'&&(
          <div>
            {sponsoring?(
              ...
              <div className="alert alert-info" style={{marginBottom:20}}>Noch kein Sponsoring-Vertrag hinterlegt.</div>
            )}
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              ...
              :<div>
                {kategorien.map(kat=>{
                  ...
                  return <div key={kat.id} style={{marginBottom:20}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}><div style={{width:10,height:10,borderRadius:'50%',background:kat.farbe}}></div><strong style={{fontSize:14}}>{kat.name}</strong></div>
                    <div style={{display:'grid',gap:8}}>
                      {katGL.map(gl=>(
        ...
        {tab==='links'&&(
          <div>
            ...
            {links.length===0?<div className="empty-state card"><p>Noch keine Links hinterlegt.</p></div>
              :<div>
                {['CI','Vertrag','Praesentation','Foto','Website','Sonstiges'].map(kat=>{
                  ...
                  const katInfo=LINK_KATEGORIEN.find(k=>k.key===kat)
                  return (
                      ...
                      <div style={{display:'grid',gap:8}}>
                        {katLinks.map(l=>(
                          <div key={l.id} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 14px',border:'1.5px solid var(--gray-200)',borderRadius:'var(--radius)',background:'var(--white)'}}>
                            <a href={l.url} target="_blank" rel="noreferrer" style={{flex:1,minWidth:0,textDecoration:'none'}} onClick={e=>e.stopPropagation()}>
              ...
              return (
                <div>
                  <div className="stats-row" style={{marginBottom:20}}>
                    <div className="stat-card blue"><div className="stat-num">{beziehungJahre||'--'}</div><div className="stat-label">Jahre in Kontakt</div></div>
                    <div className="stat-card green"><div className="stat-num">{aktivJahre||'--'}</div><div className="stat-label">Jahre aktiver Sponsor</div></div>
                    <div className="stat-card gold"><div className="stat-num" style={{fontSize:20}}>{gesamtGeld.toLocaleString('de-DE')} EUR</div><div className="stat-label">Jahresbetrag</div></div>
                    <div className="stat-card orange"><div className="stat-num">{eventCount}</div><div className="stat-label">Events{erschienen>0?' · '+erschienen+' erschienen':''}</div></div>
                  </div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
                    <div className="card">
                      <div className="section-title" style={{marginBottom:16}}>Kontakt-Aktivität</div>
                      {[['Erster Kontakt',ersterKontakt?ersterKontakt.toLocaleDateString('de-DE'):'--'],['Eintraege gesamt',historie.length],['Davon erledigt',historie.filter(h=>h.erledigt).length],['Davon offen',historie.filter(h=>!h.erledigt).length],['Ansprechpartner',ansprechpartner.length]].map(([label,value])=>(
                    ...
                    <div className="card">
                      ...
                      {sponsoring?[['Status',sponsoring.status],['Paket',sponsoring.sponsoring_pakete?.name||'Individuell'],['Jahresbetrag',sponsoring.jahresbetrag?Number(sponsoring.jahresbetrag).toLocaleString('de-DE')+' EUR':'--'],['Vertragsende',sponsoring.vertragsende?new Date(sponsoring.vertragsende).toLocaleDateString('de-DE'):'--'],['Unterzeichnet',sponsoring.vertrag_unterzeichnet?'Ja':'Nein'],['Gebuchte Leistungen',gebuchteLeistungen.length]].map(([label,value])=>(
                        <div key={label} style={{display:'flex',justifyContent:'space-between',padding:'8px 0',borderBottom:'1px solid var(--gray-100)'}}>
                          <span style={{fontSize:13,color:'var(--gray-600)'}}>{label}</span>
                          <span style={{fontSize:13,fontWeight:600}}>{value}</span>
                        </div>
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-row">
                ...
                <div className="form-group"><label>Ansprechpartner</label><select value={hForm.ansprechpartner} onChange={e=>setHForm(f=>({...f,ansprechpartner:e.target.value}))}><option value="">Kein / Allgemein</option>{ansprechpartner.map(ap=><option key={ap.id} value={ap.name}>{ap.name}{ap.position?' ('+ap.position+')':''}</option>)}</select></div>
              </div>
              ...
              {hForm.art==='Meeting'&&(
                <div style={{background:'var(--gray-100)',borderRadius:'var(--radius)',padding:14,marginBottom:4}}>
                  ...
                  <div className="form-group"><label>Ort</label><input value={hForm.meeting_ort||''} onChange={e=>setHForm(f=>({...f,meeting_ort:e.target.value}))} placeholder="z.B. Büro HC Bremen, Zoom..."/></div>
                  <div className="form-group">
                    ...
                    <div style={{border:'1.5px solid var(--gray-200)',borderRadius:'var(--radius)',padding:10,display:'flex',flexWrap:'wrap',gap:8,minHeight:44,background:'var(--white)'}}>
                      {personen.map(p=>{const selected=(hForm.meeting_teilnehmer||[]).includes(p.name);return(<button key={p.id} type="button" onClick={()=>setHForm(f=>{const current=f.meeting_teilnehmer||[];const updated=current.includes(p.name)?current.filter(n=>n!==p.name):[...current,p.name];return{...f,meeting_teilnehmer:updated}})} style={{padding:'4px 12px',borderRadius:20,border:'1.5px solid',fontSize:13,cursor:'pointer',background:selected?'var(--navy)':'var(--white)',color:selected?'white':'var(--gray-600)',borderColor:selected?'var(--navy)':'var(--gray-200)'}}>{p.name}</button>)})}
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="form-group">
                ...
                <div style={{border:'1.5px solid var(--gray-200)',borderRadius:'var(--radius)',padding:10,marginBottom:8,display:'flex',flexWrap:'wrap',gap:8,minHeight:44}}>
                  {personen.length===0?<span style={{color:'var(--gray-400)',fontSize:13}}>Unten Person hinzufuegen</span>:personen.map(p=>{const selected=(hForm.zustaendig_personen||[]).includes(p.name);return(<button key={p.id} type="button" onClick={()=>toggleZustaendig(p.name)} style={{padding:'4px 12px',borderRadius:20,border:'1.5px solid',fontSize:13,cursor:'pointer',background:selected?'var(--navy)':'var(--white)',color:selected?'var(--white)':'var(--gray-600)',borderColor:selected?'var(--navy)':'var(--gray-200)'}}>{p.name}</button>)})}
                </div>
                <div style={{display:'flex',gap:8}}>
        ...
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setEventModal(false)}>
          <div className="modal">
            ...
            <div className="modal-body">
              ...
              <div style={{display:'grid',gap:8}}>
                {alleEvents.length===0?<p style={{color:'var(--gray-400)'}}>Noch keine Events angelegt.</p>:alleEvents.map(ev=>{const selected=selectedEvents.includes(ev.id);return(<label key={ev.id} style={{display:'flex',alignItems:'center',gap:12,padding:12,border:'1.5px solid '+(selected?'var(--navy)':'var(--gray-200)'),borderRadius:'var(--radius)',cursor:'pointer',background:selected?'rgba(15,34,64,0.04)':'var(--white)'}}><input type="checkbox" checked={selected} onChange={e=>setSelectedEvents(prev=>e.target.checked?[...prev,ev.id]:prev.filter(x=>x!==ev.id))}/><div><div style={{fontWeight:600,fontSize:14}}>{ev.name}</div><div style={{fontSize:12,color:'var(--gray-400)'}}>{ev.datum?new Date(ev.datum).toLocaleDateString('de-DE'):'kein Datum'}{ev.ort?' · '+ev.ort:''}</div></div></label>)})}
              </div>
            </div>
            ...
            <div className="modal-body">
              <div className="form-group"><label>Leistung *</label>
                <select value={lForm.leistung_id} onChange={e=>{ const l=katalog.find(k=>k.id===e.target.value); setLForm(f=>({...f,leistung_id:e.target.value,preis_vereinbart:l?.preis||'',abrechnung:l?.abrechnung||'saison'})) }}>
                  ...
                  {kategorien.map(kat=>{const katL=katalog.filter(l=>l.kategorie_id===kat.id);if(katL.length===0)return null;return<optgroup key={kat.id} label={kat.name}>{katL.map(l=><option key={l.id} value={l.id}>{l.name}{l.preis?' ('+Number(l.preis).toLocaleString('de-DE')+' EUR)':''}</option>)}</optgroup>})}
                </select>
              </div>
              <div className="form-row">
                <div className="form-group"><label>Saison</label><select value={lForm.saison_id} onChange={e=>setLForm(f=>({...f,saison_id:e.target.value}))}><option value="">Keine</option>{saisons.map(s=><option key={s.id} value={s.id}>{s.name}{s.aktiv?' (aktuell)':''}</option>)}</select></div>
              ...
              <div className="form-row">
                ...
                <div className="form-group"><label>Abrechnung</label><select value={lForm.abrechnung} onChange={e=>setLForm(f=>({...f,abrechnung:e.target.value}))}><option value="saison">Pro Saison</option><option value="vertrag">Pro Vertrag</option></select></div>
              </div>
      ...
      {linkModal&&(
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setLinkModal(false)}>
          <div className="modal" style={{maxWidth:520}}>
            <div className="modal-header"><span className="modal-title">{linkForm.id?'Link bearbeiten':'Neuer Link'}</span><button className="close-btn" onClick={()=>setLinkModal(false)}>×</button></div>
            <div className="modal-body">
              <div className="form-group">
                <label>Kategorie</label>
                <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                  {LINK_KATEGORIEN.map(k=>(<button key={k.key} type="button" onClick={()=>setLinkForm(f=>({...f,kategorie:k.key}))} style={{padding:'6px 14px',borderRadius:20,border:'1.5px solid',fontSize:13,cursor:'pointer',background:linkForm.kategorie===k.key?'var(--navy)':'var(--white)',color:linkForm.kategorie===k.key?'white':'var(--gray-600)',borderColor:linkForm.kategorie===k.key?'var(--navy)':'var(--gray-200)'}}>{k.icon} {k.key}</button>))}
                </div>
              </div>
              ...
              <div className="form-group"><label>Notiz (optional)</label><input value={linkForm.notiz||''} onChange={e=>setLinkForm(f=>({...f,notiz:e.target.value}))}/></div>
            </div>
            <div className="modal-footer"><button className="btn btn-outline" onClick={()=>setLinkModal(false)}>Abbrechen</button><button className="btn btn-primary" onClick={saveLink} disabled={saving}>{saving?'Speichern...':'Speichern'}</button></div>
          </div>
        </div>
      )}
      ...
      {kontaktEditModal&&(
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setKontaktEditModal(false)}>
          <div className="modal" style={{maxWidth:600}}>
            <div className="modal-header"><span className="modal-title">Kontakt bearbeiten</span><button className="close-btn" onClick={()=>setKontaktEditModal(false)}>×</button></div>
            <div className="modal-body">
              <div className="form-row">
                <div className="form-group"><label>Firma *</label><input value={kForm.firma||''} onChange={e=>setKForm(f=>({...f,firma:e.target.value}))}/></div>
                <div className="form-group"><label>Status</label><select value={kForm.status||'Offen'} onChange={e=>setKForm(f=>({...f,status:e.target.value}))}>{statusListe.map(s=><option key={s}>{s}</option>)}</select></div>
              </div>
              <div className="form-row">
                <div className="form-group"><label>Kategorie</label><select value={kForm.kategorie||'Sponsor'} onChange={e=>setKForm(f=>({...f,kategorie:e.target.value}))}>{kategorienListe.map(k=><option key={k}>{k}</option>)}</select></div>
            ...
            <div className="modal-footer"><button className="btn btn-outline" onClick={()=>setKontaktEditModal(false)}>Abbrechen</button><button className="btn btn-primary" onClick={saveKontakt} disabled={saving}>{saving?'Speichern...':'Speichern'}</button></div>
          </div>
        </div>
      )}
    </main>
