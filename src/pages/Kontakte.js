...
export default function Kontakte() {
  ...
  async function load() {
    setLoading(true)
    const { data: k, error: ke } = await supabase.from('kontakte').select('*').order('firma')
    if (ke) console.error('Kontakte Fehler:', ke)
    setKontakte(k || [])

    const { data: p } = await supabase.from('personen').select('*').eq('aktiv', true).order('name')
    setPersonen(p || [])
  ...
  function applyFilter() {
    ...
    if (search) {
      ...
      r = r.filter(k => k.firma?.toLowerCase().includes(q) || k.branche?.toLowerCase().includes(q) || k.adresse_stadt?.toLowerCase().includes(q))
    }
  ...
  function openNew() {
    setForm(EMPTY); setLogoFile(null); setLogoPreview(null); setModal(true)
  }
          ...
          <div className="modal" style={{maxWidth:680}}>
            ...
            <div className="modal-body">
              ...
              <div className="form-row">
                ...
                <div className="form-group"><label>Branche</label>
                  <input value={form.branche||''} onChange={e=>setForm(f=>({...f,branche:e.target.value}))} placeholder="z.B. Gesundheit, IT..."/>
                </div>
              </div>
              <div className="form-row">
